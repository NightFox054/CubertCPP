...
// import frc.robot.Constants.ModuleConstants;
    ...
    SwerveModule::SwerveModule(int driveMotorID, int turnMotorID, int encoderID, frc::Rotation2d _angleOffset) :
        drive(driveMotorID),
        turn(turnMotorID),
        turnEncoder(encoderID),
    {
        ...
        frc::SmartDashboard::GetNumber("CanCoder ", driveMotorID + 2);
        initEncoder(encoderID);
        initDriveMotor(driveMotorID);
        initTurnMotor(turnMotorID);
        // feedforward = frc::SimpleMotorFeedforward(ModuleConstants.kDriveS, ModuleConstants.kDriveV,
    ...
    //TEMP POSITION (Weird formating errors if I dont move this here -_-)
